...
type SavedLink = { id: string; name: string; owner: string; repo: string; branch: string; path: string; preset: string; payload: string; createdAt: number };
// Legacy backups won't carry `path`/`preset`; normalize so openSaved never sees undefined.
const normLink = (l: any): SavedLink => ({ ...l, path: l.path || "", preset: l.preset || "" });
...
function App() {
  ...
  const [savePrompt, setSavePrompt] = useState<{ owner: string; repo: string; branch: string; path: string; preset: string; payload: string } | null>(null);
  ...
  useEffect(() => {
    ...
    setOwner(parsedOwner || DEMO.owner); setRepo(parsedRepo || DEMO.repo); setBranch(hash.get("branch") || DEMO.branch); setDemoPath(hash.get("path") || (hasDemo ? DEMO.path : "")); setPreset(hash.get("preset") || ""); setReadonly(hash.has("readonly"));
  ...
  const generate = async () => {
    ...
    const flags = token ? "" : "&public";
    const presetPart = effPreset ? `&preset=${encodeURIComponent(effPreset)}` : "";
    const value = `${appBaseHref()}#repo=${encodeURIComponent(`${owner}/${repo}`)}&branch=${encodeURIComponent(resolvedBranch)}${pathPart}${presetPart}${flags}${payload ? `&payload=${encodeURIComponent(payload)}` : ""}`;
    setLink(value);
    ...
    setSavePrompt(token ? { owner, repo, branch: resolvedBranch, path: root, preset: effPreset, payload } : null);
    setSaveName(`${owner}/${repo} · ${resolvedBranch}`);
  };
  const weakPin = pin.length < 8 || /^\d+$/.test(pin);
  const savedLinkUrl = (l: SavedLink) => `${appBaseHref()}#repo=${encodeURIComponent(`${l.owner}/${l.repo}`)}&branch=${encodeURIComponent(l.branch)}${l.path ? `&path=${encodeURIComponent(l.path)}` : ""}${l.preset ? `&preset=${encodeURIComponent(l.preset)}` : ""}&payload=${encodeURIComponent(l.payload)}`;
  const saveLink = () => {
    if (!savePrompt) return;
    const entry: SavedLink = { id: crypto.randomUUID?.() || String(Date.now()), name: saveName.trim() || `${savePrompt.owner}/${savePrompt.repo} · ${savePrompt.branch}`, owner: savePrompt.owner, repo: savePrompt.repo, branch: savePrompt.branch, path: savePrompt.path || "", preset: savePrompt.preset || "", payload: savePrompt.payload, createdAt: Date.now() };
  ...
  const openSaved = (l: SavedLink) => { window.location.hash = savedLinkUrl(l).split("#")[1]; setOwner(l.owner); setRepo(l.repo); setBranch(l.branch); setDemoPath(l.path || ""); setPreset(l.preset || ""); setReadonly(false); setMode("unlock"); };
  ...
  if (mode === "viewer") return <Viewer repo={repoLabel} branch={branch} path={demoPath} token={token} preset={preset} readonly={readonly} onExit={() => setMode("home")} />;
...
function Viewer({ repo, branch, path, token, preset, readonly, onExit }: { repo: string; branch: string; path: string; token: string; preset: string; readonly: boolean; onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    navigator.serviceWorker.register(`${baseRoot}edgeqa-sw.js`, { type: "module" })
      ...
      .then(async (registration) => {
        ...
        if (preset) registration.active?.postMessage({ type: "SET_PRESET", scope: `${repo}/${branch}`, preset });
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (d.type === "EDGEQA_COMPILE_REQUEST") {
        ...
        (async () => {
          ...
          try {
            if (d.preset === "svelte") { code = await compileSvelteText(d.code, d.path); ok = true; }
            else if (d.preset === "vue") { code = await compileVueText(d.code, d.path); ok = true; }
  ...
  }, [repo, branch, baseRoot, token, preset]);
